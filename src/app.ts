...
async function main(): Promise<void> {
  ...
  if (!fs.existsSync(config.keyLocation)) {
    console.log(
      'No private key found at the location specified in the configuration file. Exiting...'
    );
    return;
  }
  ...
  const clientAssertionJwtPayload: jwt.JwtPayload = Utils.buildClientAssertionJwtPayload(
    encodedAuditJwt,
    config
  );
  ...
  await fetch(config.serviceURL, eivRequestOptions)
    ...
    .then((res: EivResponseBody) => {
      Utils.logVerbose(
        `Response from ${config.serviceURL} (ANPR - Esistenza In Vita)`,
        res,
        config
      );
      console.log(
