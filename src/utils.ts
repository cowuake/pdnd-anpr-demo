...
export function buildClientAssertionJwtPayload(encodedAuditJwtToken: string, config: Config): JwtPayload {
  const [issued, expires] = computeJwtValidityInterval(config.tokenValidityInMinutes)
  return {
    sub: config.clientID,
    jti: generateJti(),
...
export function buildIntegrityJwtPayload(digest: string, config: Config): JwtPayload {
  ...
  return {
    ...
    jti: generateJti(),
    iat: issued,
    signed_headers: [{ digest }, { Accept: 'application/json' }, { 'Content-Type': 'application/json' }],
    nbf: issued,
    exp: expires,
    iss: config.clientID,
    aud: config.signatureAudience
